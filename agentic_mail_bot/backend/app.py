...
#     message: str
#     user_id: str
#     intent: Literal["chat", "add", "email", "confirm_email", "search"]
#     response: str
...
#             f"<a href='/auth/google?user_id={state['user_id']}' target='_blank' "
...
#     if not email_match:
#         state["response"] = "❌ Please provide recipient email address."
#         return state
...
#     if token:
#         prompt = f"""
#         The user has just successfully connected their Gmail account.
#         Respond professionally confirming connection and tell them
#         they can now send emails.
#         """
...
#     draft = EMAIL_DRAFTS.get(state["user_id"])
...
#     del EMAIL_DRAFTS[state["user_id"]]

#     state["response"] = "✅ Email sent successfully via Gmail API!"
#     return state
...
# for n in ["chat", "add", "email", "confirm_email", "search","gmail_connected"]:
#     graph.add_edge(n, END)
...
async def email_node(state: AgentState):
    ...
    #     return state
    # if len(emails) == 1:
    #     greeting_line = f"Dear {extracted_name(emails[0])},"
    # else:
    #     greeting_line = "Dear Team,"

    # to_email = ", ".join(emails)   # Gmail supports comma separated emails
...
#         # 🔥 Extract actual response from MCP
#         if hasattr(result, "data"):
#             final_message = str(result.data)
#         elif hasattr(result,"content"):
#             final_message=str(result.content)
#         else:
#             final_message = str(result)

#         # Only delete draft if actually sent
#         if any(word in final_message.lower() for word in ["sent", "success", "ok"]):
#             del EMAIL_DRAFTS[state["user_id"]]
